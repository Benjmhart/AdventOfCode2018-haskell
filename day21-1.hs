...
-- this just runs the program, see textFiles for answer

main :: IO()
main = do
  input <- readFile $ "./inputs/day21-1.txt"
...
solve args@((PS reg p t), is)
  | t > 30 = args
  | IM.size is <= (t' ((show $ reg !! p) ++ (show reg)) $ reg !! p) = args
...
parse (lines -> xs) = ((PS [12145300,0,0,0,0,0] p 0), is)
  where
    p  = read . (:[]) . last . head $ xs 
